 ...
 */
@Service
public class RetellServiceTemplate {
    
    private static final Logger logger = LoggerFactory.getLogger(RetellServiceTemplate.class);
    ...
    public RetellServiceTemplate(UserService userService) {
    ...
    private CallResponse convertToCallResponse(CallRequest callRequest, JsonNode retellResponse) {
        ...
        if (retellResponse.has("call_id")) {
            response.setCallId(retellResponse.get("call_id").asText());
        }
