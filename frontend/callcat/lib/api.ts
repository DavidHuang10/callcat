...
class ApiService {
  ...
  async getCalls(status?: string, limit: number = 20): Promise<CallListResponse> {
    const params = new URLSearchParams()
    if (status) params.append('status', status)
    params.append('limit', limit.toString())
    
    return this.request<CallListResponse>(`/api/calls?${params.toString()}`)
  }
  ...
  async updateCall(callId: string, data: UpdateCallRequest): Promise<CallResponse> {
    return this.request<CallResponse>(`/api/calls/${callId}`, {
      method: 'PUT',
      body: JSON.stringify(data)
    })
  ...
  async updateUserPreferences(data: UpdatePreferencesRequest): Promise<UserPreferencesResponse> {
    return this.request<UserPreferencesResponse>('/api/user/preferences', {
      method: 'PUT',
      body: JSON.stringify(data)
    })
  ...
  // Authentication
  async login(email: string, password: string): Promise<{ token: string; userId: string; email: string; fullName: string; expirationTime: number }> {
    const response = await this.request<{ token: string; userId: string; email: string; fullName: string; expirationTime: number }>('/api/auth/login', {
      method: 'POST',
  ...
  async register(
    ...
    firstName: string, 
    lastName: string
  ): Promise<{ token: string; userId: string; email: string; fullName: string; expirationTime: number }> {
    const response = await this.request<{ token: string; userId: string; email: string; fullName: string; expirationTime: number }>('/api/auth/register', {
      method: 'POST',
